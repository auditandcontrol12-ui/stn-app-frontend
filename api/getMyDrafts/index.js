  ...
  handler: async (request, context) => {
    try {
      ...
      const req = pool.request()
        .input("CreatedByEmail", sql.NVarChar(1020), (sessionUser.UserEmail || "").trim().toLowerCase());
      ...
      const result = await req.query(`
        ...
        WHERE LOWER(LTRIM(RTRIM(h.CreatedByEmail))) = @CreatedByEmail
          AND h.Status = 'Draft'
          AND h.IsDeleted = 0
          ${areaFilterSql}
        ORDER BY
      ...
      return {
        ...
        jsonBody: {
          success: true,
          currentUserEmail: sessionUser.UserEmail,
          draftCount: result.recordset.length,
